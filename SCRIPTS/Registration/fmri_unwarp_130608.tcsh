...
# convert epi to nii
####################
	$RELEASE/nifti_4dfp -n $epi unwarp/$epi:t
################
# run epi_unwarp
################

	#sanity check the dwell
	if(`echo $dwell | awk '{if($1 < 0.01) print("1"); else print("0");}'` == 1) then
		echo "DWELL seems to be in seconds, converting to ms."
		set dwell = `echo $dwell | awk '{printf("%3.15f",$1*1000)}'`
	endif

	$PP_SCRIPTS/SurfacePipeline/epi_unwarp_corbetta -m $mag -p $phase -e unwarp/$epi:t -dwell $dwell -te $te -delta $delta -r $patid -dir $ped -nomask
...
#############################
	...
	$RELEASE/nifti_4dfp -4 unwarp/${patid}_epi_unwarped unwarp/${epi:t}_uwrp -N
	exit $status
	...
	$RELEASE/nifti_4dfp -4 unwarp/${epi:t}_uwrp unwarp/${epi:t}_uwrp -N
	exit $status
...
#################
	if ( $#argv == 10 ) then
		...
		$RELEASE/nifti_4dfp -n $t2w unwarp/$t2w:t
		$FSLDIR/bin/bet unwarp/$t2w:t unwarp/${t2w:t}_brain -m -n -f 0.4 -R
		gunzip -f unwarp/${t2w:t}_brain_mask.nii.gz
		$RELEASE/nifti_4dfp -4 unwarp/${t2w:t}_brain_mask unwarp/${t2w:t}_brain_mask -N
...
#####################################################
	...
	$RELEASE/t4img_4dfp unwarp/${mean:t}_to_${epi:t}_t4	unwarp/${basis:t} unwarp/${basis:t}_on_${epi:t} -O$epi
	if ($status) exit $status
####################
# convert epi to nii
####################
...
###################################
	...
	echo "set unwarp_Tyler = ${PP_SCRIPTS}/SurfacePipeline/unwarp_eta.tcsh" >> unwarp/${epi:t}_basis_opt.params
##################################
...
###############################################
	$RELEASE/basis_opt unwarp/${epi:t}_basis_opt.params unwarp/${epi:t}_basis_opt.weights -n3 -e0.01
####################################
...
# convert the shiftwarp to an absolute warp
###########################################
	$FSLDIR/bin/convertwarp -s unwarp/${epi:t}_uwrp_shift_map -r unwarp/$epi:t --shiftdir=$ped -o unwarp/epi_0_unwarp_shift_warp
#################################
