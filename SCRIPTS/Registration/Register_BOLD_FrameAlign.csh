	...
	foreach Run($RunIndex)	#these are labels if nifti and numbers if dicom
		...
		cd bold${Run}
			...
			if($?NORDIC_BOLD) then
				...
				matlab -nodesktop -nosplash -r "addpath(genpath('${PP_SCRIPTS}/spm12'));addpath(genpath('${PP_SCRIPTS}/matlab_scripts'));ARG.noise_volume_last=${NORDIC_BOLD_NoiseVol};ARG.temporal_phase=1;ARG.phase_filter_width=10; NIFTI_NORDIC_V2('bold${Run}_upck.nii.gz','${SubjectHome}/dicom/${NORDIC_BOLD[$Run]}','bold${Run}_upck.nii.gz',ARG);exit" || exit $status
				...
				matlab -nodesktop -nosplash -r "addpath(genpath('${PP_SCRIPTS}/spm12'));addpath(genpath('${PP_SCRIPTS}/matlab_scripts'));ARG.magnitude_only = 1; NIFTI_NORDIC_V2('bold${Run}_upck.nii.gz','bold${Run}_upck.nii.gz','bold${Run}_upck.nii.gz',ARG);exit" || exit $status
