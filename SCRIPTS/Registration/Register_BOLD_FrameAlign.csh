...
pushd $ScratchFolder/${patid}
	...
	if($#ME_ScanSets > 0) then
		...
		echo "Data is single echo, using RunIndex list for NORDIC."
		set InitialBOLDs = ($RunIndex)
	endif
	
	#run covert data to 4dfp and run NORDIC if needed
	foreach Run($InitialBOLDs)	#these are labels if nifti and numbers if dicom
	...
	if($#ME_ScanSets > 0) then
		...
		while($i <= $#ME_ScanSets)
			...
			#set Scan = $ME_ScanSet[$i]
			#set EchoIndex = ($EchoIndex $i)

			set scan_indices = `echo $ME_ScanSets[$i] | sed 's/,/ /g'`
			set ScanList = ()
			mkdir me_scan_${i}

			foreach Scan($scan_indices)
				set ScanList = ($ScanList bold${Scan}/bold${Scan}_upck.4dfp.img)
			end

			echo    MEfmri_4dfp -E$#BOLD_TE -T $BOLD_TE $ScanList -r$ME_reg -ome_scan_${i}/bold${i}_upck_ME -e30
                        MEfmri_4dfp -E$#BOLD_TE -T $BOLD_TE $ScanList -r$ME_reg -ome_scan_${i}/bold${i}_upck_ME -e30
			if($status) then
				echo "SCRIPT: $0 : 00002.1 : $2 Unable to combine multi echo data set $Scan." ${DebugFile}
	                        exit 1
			endif

			foreach bold($scan_indices)
				mv bold${bold} me_scan_sets/
			end

			mv me_scan_${i} bold${i}
			pushd bold${i}
				cp -s bold${i}_upck_ME_Swgt.4dfp.img bold${i}_upck.4dfp.img
				cp -s bold${i}_upck_ME_Swgt.4dfp.hdr bold${i}_upck.4dfp.hdr
				cp -s bold${i}_upck_ME_Swgt.4dfp.ifh bold${i}_upck.4dfp.ifh
				cp -s bold${i}_upck_ME_Swgt.4dfp.img.rec bold${i}_upck.4dfp.img.rec
			popd
	...
	#we should now be back to single echo runs. Use RunIndex going forward.
	#slice timing and frame alignment
	...
	foreach Run($RunIndex)			#add this bold run to the list to be included in cross run alignment and anat_ave
	...
	foreach Run($xr3d_Runs)
		...
		cp bold${Run}/bold${Run}_upck_faln_dbnd_xr3d.mat bold${Run}/bold${Run}_upck_faln_dbnd_xr3d_nf.mat

		$RELEASE/mat2dat bold${Run}/bold${Run}_upck_faln_dbnd_xr3d_nf.mat -I -R -D
		if($status) exit 1
		$RELEASE/mat2dat bold${Run}/bold${Run}_upck_faln_dbnd_xr3d_nf -I
		if($status) exit 1

		mv bold${Run}/*"_xr3d".*dat bold${Run}/*"_xr3d.mat" bold${Run}/*_upck_faln_dbnd_xr3d_dat.* bold${Run}/*"_xr3d_nf".*dat bold${Run}/*"_xr3d_nf.mat" bold${Run}/*_upck_faln_dbnd_xr3d_nf_dat.* ${SubjectHome}/Functional/Movement
	end
