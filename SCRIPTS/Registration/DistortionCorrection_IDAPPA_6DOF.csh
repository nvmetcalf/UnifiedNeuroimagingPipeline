...
if($#ImageStack < 2) then
	echo "	ERROR: Cannot perform image derived distortion correction on less that image sets."
	exit 1
endif

if($#ped < 2) then
	echo "	ERROR: Image derived distortion correction requires at least 2 opposing phase encoding directions."
	exit 1
endif
	...
	foreach ImageStack($seImageStack_list)
		@ dim = `fslinfo $ImageStack | grep dim1 | head -1 | awk '{print $2}'` * `fslinfo $ImageStack | grep dim2 | head -1 | awk '{print $2}'` * `fslinfo $ImageStack | grep dim4 | head -1 | awk '{print $2}'`
	...
	foreach ImageStack($seImageStack_list)
		...
		flirt -in $ImageStack -ref $target_ImageStack -out `basename $ImageStack:r:r"_reg.nii.gz"` -dof 6 -interp spline
		if($status) exit 1
