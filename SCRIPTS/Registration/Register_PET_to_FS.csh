...
if($?FDG && -e ${SubjectHome}/Anatomical/Volume/FDG/${patid}_FDG_to_${patid}_T1.mat) then
	set modes_available = ($modes_available FDG)
endif
	
if($?O2 && -e ${SubjectHome}/Anatomical/Volume/O2/${patid}_O2_to_${patid}_T1.mat) then
	set modes_available = ($modes_available O2)
endif
	
if($?CO && -e ${SubjectHome}/Anatomical/Volume/CO/${patid}_CO_to_${patid}_T1.mat) then
	set modes_available = ($modes_available CO)
endif
	
if($?H2O && -e ${SubjectHome}/Anatomical/Volume/H2O/${patid}_H2O_to_${patid}_T1.mat) then
	set modes_available = ($modes_available H2O)
endif
		
if($?PIB && -e ${SubjectHome}/Anatomical/Volume/PIB/${patid}_PIB_to_${patid}_T1.mat) then
	set modes_available = ($modes_available PIB)
endif

if($?TAU && -e ${SubjectHome}/Anatomical/Volume/TAU/${patid}_TAU_to_${patid}_T1.mat) then
	set modes_available = ($modes_available TAU)
endif

foreach mode($modes_available)
	convert_xfm -omat ${SubjectHome}/Anatomical/Volume/$mode/${patid}_${mode}_to_${patid}_orig.mat -concat ${SubjectHome}/Masks/FreesurferMasks/${patid}_T1_to_${patid}_orig.mat ${SubjectHome}/Anatomical/Volume/${mode}/${patid}_${mode}_to_${patid}_T1.mat
	if($status) exit 1
