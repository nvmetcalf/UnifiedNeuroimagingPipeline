...
pushd ${SubjectHome}/Anatomical/Volume/T1
	...
	if($status) then
 	...
 	fast -t 1 -n 3 -H 0.1 -I 4 -l 20.0 -b -B ${patid}_T1_brain.nii.gz
 	if($status) then
 	...
 	fslmaths ${patid}_T1_native.nii.gz -div ${patid}_T1_brain_bias.nii.gz ${patid}_T1.nii.gz
 	if($status) exit 1
 	
 	set BC_min_max = (`fslstats ${patid}_T1_brain_restore.nii.gz -n -R`)
 	if(`echo $BC_min_max[2] | awk '{if($1 == 0) print 0}'` == 0) then
