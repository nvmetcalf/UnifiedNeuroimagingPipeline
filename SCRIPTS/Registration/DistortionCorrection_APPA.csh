...
pushd ${SubjectHome}/Anatomical/Volume/FieldMapping_${FM_Suffix}
	...
	#get the total readout for the AP image. MUST be the same for PA
	...
	bet fmap_mag_${direction} fmap_${direction}_mag_brain -f 0.2
	if($status) exit 1
	...
	foreach direction($peds)
		...
		$PP_SCRIPTS/Registration/epi_reg_abs --echospacing=$dwell[1] --fmap=fmap_rads_${direction}.nii.gz --fmapmag=fmap_mag_${direction}.nii.gz --fmapmagbrain=fmap_mag_${direction}_brain.nii.gz --pedir=$direction --epi=${patid}_${FM_Suffix}_ref_distorted_${direction} --t1=${Target_Path}/${Reg_Target}/${Target_Patid}_${Reg_Target} --t1brain=${Target_Path}/${Reg_Target}/${Target_Patid}_${Reg_Target}_brain_restore.nii.gz --out=${patid}_${FM_Suffix}_ref_unwarped_${direction} --noclean
		if($status) exit 1
		...
		convertwarp -o ${SubjectHome}/Anatomical/Volume/FieldMapping_${FM_Suffix}/${patid}_${FM_Suffix}_ref_unwarped_${direction}_warp.nii.gz -r ${Target_Path}/${Reg_Target}/${Target_Patid}_${Reg_Target} --premat=${SubjectHome}/Anatomical/Volume/FieldMapping_${FM_Suffix}/${patid}_${FM_Suffix}_ref_unwarped_${direction}_fieldmap2str.mat
		if($status) exit 1
	end
popd
exit 0
