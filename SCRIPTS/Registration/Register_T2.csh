...
pushd ${SubjectHome}/Anatomical/Volume/T2
	...
	#extract the brain from the T1
 	fast -b -B -I 10 -l 10 -t 2 ${patid}_T2_brain.nii.gz
 	if($status) then
 		decho "Failed to complete bias correction on T2."
 		exit 1
 	endif
 	
	$FSLBIN/flirt -in ${patid}"_T2" -ref ../T1/${patid}_T1 -omat ${patid}_T2_to_${patid}_T1.mat -dof 6 -interp spline -cost mutualinfo -searchcost mutualinfo
	if($status) then
	...
	if($MaximumRegDisplacement != 0) then
		flirt -in ../T1/${patid}_T1 -ref ${patid}"_T2" -omat ${patid}"_T2"_to_${patid}_T1_rev.mat -dof 6 -cost mutualinfo -searchcost mutualinfo
		if($status) exit 1
