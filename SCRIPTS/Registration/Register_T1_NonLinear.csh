...
pushd ${SubjectHome}/Anatomical/Volume/T1
	...
	fnirt --in=${patid}"_T1_brain" --ref=${target} --refmask=${target}_brain_mask --inmask=${patid}"_T1_brain_mask" --jout=${cwd}/$patid"_jacobiantransform.nii.gz" --fout=${cwd}/$patid"_T1_to_${AtlasName}_warpfield_111.nii.gz" --aff=${patid}_T1_to_${AtlasName}.mat --cout=${cwd}/$patid"_T1_to_${AtlasName}_coeffield_111.nii.gz" --config=${target}.cnf
	if($status) then
