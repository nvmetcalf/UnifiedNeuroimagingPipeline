...
pushd ${SubjectHome}/Anatomical/Volume/T1
	...
	$FSLBIN/fnirt --in=${patid}"_T1" --ref=${target} --inmask=$SubjectHome/Masks/${patid}_used_voxels_T1.nii.gz --jout=${cwd}/$patid"_jacobiantransform.nii.gz" --fout=${cwd}/$patid"_T1_warpfield_111.nii.gz" --aff=${patid}_T1_to_${AtlasName}.mat --cout=${cwd}/$patid"_T1_coeffield_111.nii.gz" --config=${target}.cnf
	if($status) then
