...
foreach direction($peds)
	...
	if($?try_mask) then
		...
		#see if we want to check how far a voxel displaces
		if($MaximumRegDisplacement == 0) then
			set MaximumRegDisplacement = `fslinfo ${patid}_${FM_Suffix}_ref_unwarped_${direction}_brain.nii.gz | grep pixdim | awk '{print $2 * 1.25}' | sort -u | tail -1`
		endif
