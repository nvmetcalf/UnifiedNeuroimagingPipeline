...
pushd ${SubjectHome}/Anatomical/Volume/FieldMapping_${FM_Suffix}
	...
	if(! $?day1_path || ! $?day1_patid) then
		...
		if ($#fm < 3) then
			echo "SCRIPT: $0 : 00004 : ERROR: required measured field map correction variables not set"
			exit 1
		endif
	...
	foreach direction($peds)
		...
		#see if we want to check how far a voxel displaces
		if($MaximumRegDisplacement == 0) then
			set MaximumRegDisplacement = `fslinfo ${patid}_${FM_Suffix}_ref_distorted_${direction}_uwrp.nii.gz | grep pixdim | awk '{print $2 * 1.25}' | sort -u | tail -1`
		endif
