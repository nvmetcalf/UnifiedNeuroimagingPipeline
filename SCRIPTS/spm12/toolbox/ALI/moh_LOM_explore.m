...
%
% =====================================================
% Mohamed Seghier, 23.11.2008, updated 07.04.2014
%
...
switch todo
    case 'list_only'
        ...
        vox_mm = spm_orthviews('Pos');
        vox = round(inv(v.mat)*[vox_mm ; 1]) ;
        
        ii = sub2ind(v.dim, vox(1), vox(2), vox(3)) ;
        iii = find(LOM.index.voxels == ii) ;
        if isempty(iii)
            disp('!!!!! No lesion found at this brain location !!!!!')
        else
            disp(['#### List of subjects with lesion at [mm]: ',...
                num2str(round(vox_mm)')]) ;
            list_index = find(LOM.index.occurence(iii, :)) ;
            disp(LOM.subjects(list_index, :)) ;
            disp('-----------------------------------------------------')
            if length(list_index) < size(LOM.subjects,1)
                disp(['#### List of subjects with NO lesion at [mm]: ',...
                    num2str(round(vox_mm)')]) ;
                disp(LOM.subjects(setdiff(1:size(LOM.subjects,1),list_index), :)) ;
            end
        end
        disp('  ')
    ...
    case 'list_only_voi'
        ...
        if ~isempty(list_index)
            disp(['#### List of subjects with lesion within a '...
                ...
                ' centred at [mm]: ' num2str(round(vox_mm)')]) ;
            disp(LOM.subjects(list_index, :)) ;
            disp('-----------------------------------------------------')
            if length(list_index) < size(LOM.subjects,1)
                disp(['#### List of subjects with NO lesion within a '...
                    num2str(rad_mm) 'mm-radius sphere'...
                    ' centred at [mm]: ' num2str(round(vox_mm)')]) ;
                disp(LOM.subjects(setdiff(1:size(LOM.subjects,1),list_index), :)) ;
            end            
        else
            disp('!!!!! No lesion found within this VOI !!!!!') ;
        end
        disp('  ')
    ...
    otherwise
        disp('=== Oooops: option not supported yet........!!!!') ;
end
