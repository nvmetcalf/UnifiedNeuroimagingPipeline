...
segment_subregions thalamus --cross ${FreesurferVersionToUse} --sd $SUBJECTS_DIR
if($status) then
	decho "		FAILED! ${patid} failed to segment thalamus." $DebugFile
	exit 1
endif

segment_subregions hippo-amygdala --cross ${FreesurferVersionToUse} --sd $SUBJECTS_DIR
if($status) then
	decho "		FAILED! ${patid} failed to segment hippo-amygdala" $DebugFile
	exit 1
endif

segment_subregions brainstem --cross ${FreesurferVersionToUse} --sd $SUBJECTS_DIR
if($status) then
