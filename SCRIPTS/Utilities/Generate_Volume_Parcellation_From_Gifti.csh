...
if(! -e $SUBJECTS_DIR/Freesurfer) then
	echo "$SUBJECTS_DIR/Freesurfer does not exist."
	exit 1
endif

if (! -e ${ParticipantsFolder}/Freesurfer/mri/gtmseg.mgz) then
	echo "gtmseg has not been completed. Unable to continue."
	exit 1
endif

if(! -e ${ParticipantsFolder}/Anatomical/Surface/fsaverage_LR32k/${patid}.L.midthickness.32k_fs_LR.surf.gii) then
...
pushd PET/Parcellations/$ParcellationName
	...
	mri_convert $SUBJECTS_DIR/Freesurfer/mri/orig.mgz orig.nii.gz
	if($status) exit 1
	set orig_image = $cwd/orig.nii.gz
			
	wb_command -label-to-volume-mapping $Parcellation_LHlabel ${ParticipantsFolder}/Anatomical/Surface/fsaverage_LR32k/${patid}.L.midthickness.32k_fs_LR.surf.gii $orig_image LH_parcellation.nii.gz -ribbon-constrained ${ParticipantsFolder}/Anatomical/Surface/fsaverage_LR32k/${patid}.L.white.32k_fs_LR.surf.gii ${ParticipantsFolder}/Anatomical/Surface/fsaverage_LR32k/${patid}.L.pial.32k_fs_LR.surf.gii 
	if($status) exit 1
			
	wb_command -label-to-volume-mapping $Parcellation_RHlabel ${ParticipantsFolder}/Anatomical/Surface/fsaverage_LR32k/${patid}.R.midthickness.32k_fs_LR.surf.gii $orig_image RH_parcellation.nii.gz -ribbon-constrained ${ParticipantsFolder}/Anatomical/Surface/fsaverage_LR32k/${patid}.R.white.32k_fs_LR.surf.gii ${ParticipantsFolder}/Anatomical/Surface/fsaverage_LR32k/${patid}.R.pial.32k_fs_LR.surf.gii
	if($status) exit 1
	...
	lta_convert --inlta ${SUBJECTS_DIR}/Freesurfer/mri/gtmseg.lta --outlta gtm_to_orig.lta --invert
	if($status) exit 1
	...
	flirt -in $ParcellationName"_orig" -ref ${ParticipantsFolder}/Masks/FreesurferMasks/${patid}_orig_to_${patid}_T1 -out $ParcellationName"_T1" -init ${ParticipantsFolder}/Masks/FreesurferMasks/${patid}_orig_to_${patid}_T1.mat -applyxfm -interp nearestneighbour
	if($status) exit 1 
	
	mri_vol2vol --mov $ParcellationName"_orig.nii.gz" --lta ${SUBJECTS_DIR}/Freesurfer/mri/gtmseg.lta --targ gtmseg+wmparc.nii.gz --o "${ParcellationName}_gtm.nii.gz" --nearest
	if ($status) exit 1
