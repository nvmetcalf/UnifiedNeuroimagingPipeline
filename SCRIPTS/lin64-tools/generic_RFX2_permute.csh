...
set idstr = '$Id: generic_RFX2_permute.csh,v 1.3 2020/11/26 05:31:32 avi Exp $'
echo $idstr
...
echo	imgopr_4dfp -R	-a${grp1}_surr_N -l$nst1
	imgopr_4dfp -R	-a${grp1}_surr_N -l$nst1		> /dev/null
	if ($status) exit $status
echo	imgopr_4dfp -R	-a${grp2}_surr_N -l$nst2
	imgopr_4dfp -R	-a${grp2}_surr_N -l$nst2		> /dev/null
	if ($status) exit $status

echo	RFX_4dfp $lst1 ${grp1}_surr_N $lst2 ${grp2}_surr_N -R
