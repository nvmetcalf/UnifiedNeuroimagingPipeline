...
set idstr = '$Id: generic_RFX1_permute.csh,v 1.4 2020/11/25 23:45:18 tanenbauma Exp $'
echo $idstr
...
set grp1_zfrm	= (`cat $grp1_zfrm | sed -r 's|\.4dfp{0,1}(\.img)$||'`)
set grp1_dfnd	= (`cat $grp1_dfnd | sed -r 's|\.4dfp{0,1}(\.img)$||'`)
...
while ($k <= $n)
	...
	echo surr/${F}_surr	>> $lstc	# surrogate data filenames remain constant
	@ k++
end
echo cat $lstc
cat $lstc
...
################################################
# copy data to ./surr  and pseudorandomly flip sign
################################################
	@ k = 1
	while ($k <= $n)
		set F = $grp1_zfrm[$k]
		foreach e (img ifh)
			/bin/cp $F.4dfp.$e surr/${F:t}_surr.4dfp.$e
		end
		@ seed = $m + $k
		set perm = (`permuteN 2 -s$seed`)
		if ($perm[1] % 2) then
			scale_4dfp surr/${F:t}_surr -1	> /dev/null
			if ($status) exit $status
		endif
		@ k++
	end
