...
class DataExtractor(object):
            ...
            try:
                ...
                try:
                    ...
                    for field in Definitions.DICOM_TAGS:
                        tag_to_extract = Definitions.DICOM_TAGS[field]
                        if (not tag_to_extract in self.dicom_metadata) and tag_to_extract in json_data:
                            self.dicom_metadata[tag_to_extract] = json_data[tag_to_extract]
    ...
    def guess_date_acquired_from_path(self, 
        ...
        if Definitions.DICOM_TAGS['ACQUISITION_TIME'] in self.dicom_metadata:
            date_acquired = self.dicom_metadata[Definitions.DICOM_TAGS['ACQUISITION_TIME']]
        ...
        for scan_source in project_scan_locations:
            scan_root_dir = os.path.join(self.__projects_data[Definitions.SCANS_DIR], scan_source)
    ...
    def guess_software_version_from_path(self) -> str:
        ...
        if not Definitions.DICOM_TAGS['SOFTWARE_VERSION'] in self.dicom_metadata:
            print('Could not extract software version from json metadata.')
            return ''
        
        return self.dicom_metadata[Definitions.DICOM_TAGS['SOFTWARE_VERSION']]
