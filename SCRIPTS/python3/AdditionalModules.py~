...
def check_dicom_files(regex_match, this):
    ...
    #Now lets check to make sure all the exensions are the same.
    extensions = [ '.'.join(ext.split('.')[1:]) for ext in file_name_list ]
    
    valid_extensions = this.boundary_data['Allowed_Dicom_Scan_Extensions']
    for fname,extension in zip(file_name_list,extensions):
...
def check_sub_ses(regex_match, this):
    
    #The match groups for the file names are 7 if surrounded by parentheses (will also pull out the qoutes so these 
    #need to be removed) and group 5 if there is only one value.
    sub_ses = regex_match.group(5)

    #Havent figued out how to do this for all shell variables set. For now if there is a 
    #shell variable in the file name skip it. 
    if sub_ses.find('$') != -1:
        return 0
