    ...
    for ArgIn = 1:length(varargin)
        ...
        && ~strcmp(varargin{ArgIn}, 'int16')  && ~strcmp(varargin{ArgIn}, 'int8'))
            ...
            try
                ...
                [fid message] = fopen(img_4dfp_name,'r');
            end
            ...
            switch(Cast)
                ...
                otherwise
                    Current_4dfp.voxel_data = single(fread(fid, 'float'));
            end
            ...
            if ~isequal(read_volumes, Current_4dfp.ifh_info.matrix_size(4))
                disp('WARNING: ifh volumes and img volumes do not match! ifh may not be appropriate.');
                %img_4dfp = [];
                %return;
            end
