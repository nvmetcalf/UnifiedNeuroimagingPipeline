...
if ~isfield(ARG,'DIROUT') % Specify where to save data
    [outdir filename ext] = fileparts(fn_out);
    %if isfolder(outdir)
    %    ARG.DIROUT=[outdir '/'];
    %else
    ARG.DIROUT=[pwd '/'];
...
if ARG.magnitude_only~=1
    ...
    if ARG.use_generic_NII_read~=1
        ...
        info_phase=load_nifti(fn_phase_in);
        info=load_nifti(fn_magn_in);
        I_M=abs(single(info.vol));
        I_P=single(info_phase.vol);
    ...
    % convert to double and then scale the phase
    I_P = double(I_P);
    range_norm=phase_range-phase_range_min;
    range_center=(phase_range+phase_range_min)/range_norm*1/2;
    I_P = (double(I_P)./range_norm -range_center)*2*pi;
    II=double(I_M)  .* exp(1i*I_P);
    
    
    if 0
        if strmatch(info_phase.Datatype,'uint16')
            I_P = double(I_P)/phase_range*2*pi;
            II=double(I_M)  .* exp(1i*I_P);
        elseif strmatch(info_phase.Datatype,'int16')
            I_P = (double(I_P)+1-(phase_range+1)/2)/(phase_range+1)*2*pi;
            II=double(I_M)  .* exp(1i*I_P);
        elseif strmatch(info_phase.Datatype,'double')
    ...
    if ARG.use_generic_NII_read~=1
        info=load_nifti(fn_magn_in);
        I_M=abs(double(info.vol));
        clear tmp;
    else
        tmp=load_nii(fn_magn_in);
        I_M=abs(double(tmp.img));
    end
...
if ( ARG.save_gfactor_map==2 )  | ( ARG.save_gfactor_map==1 )
    ...
    elseif strmatch(info.Datatype,'int16')
        ...
        g_IMG= double(abs(g_IMG)*2^gain_level);
    end
    
    info.vol = g_IMG;
    save_nifti(info, [ARG.DIROUT 'gfactor_' fn_out(1:end) '.nii.gz'])
...
if isfield(ARG,'make_complex_nii')
    ...
    elseif strmatch(info.Datatype,'int16')
        ...
        IMG2_tmp= double(abs(IMG2_tmp)*2^gain_level);
    end
    
    info.vol = IMG2_tmp;
    save_nifti(info,[ARG.DIROUT fn_out 'magn.nii.gz'])
    ...
    info_phase.vol = IMG2_tmp;
    save_nifti(info_phase,[ARG.DIROUT fn_out 'phase.nii.gz'])
...
%     end
    if ARG.use_generic_NII_read==0;
        info.vol = IMG2(:,:,:,1:end-ARG.noise_volume_last);
        info.dim(5) = info.dim(5) - ARG.noise_volume_last;
        save_nifti(info,[ARG.DIROUT fn_out(1:end) '.gz'])
    else
        nii=make_nii(IMG2);
