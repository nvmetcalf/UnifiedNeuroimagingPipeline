function ft_write_cifti_mod(filename, source, varargin)
...
dimord = getdimord(source, 'dimord');
...
if any(strcmp(dimtok, 'pos'))
  ...
  for i=1:length(BrainStructurelabel)
    ...
    if all(ModelType(sel)==find(strcmp(ModelTypelabel, 'VOXEL')))
      ...
      branch = branch(end);
      tmp = source.pos(sel,:);
      if ~isfield(source,'transform');source.transform = [0.3 0 0 1;0 0.3 0 1; 0 0 0.3 1; 0 0 0 0];end % JSS 11/4/2015      
      ...
      SurfaceNumber = find(sel, 1, 'last')-SurfaceNumber;
      tree = attributes(tree, 'add', branch, 'IndexOffset', printwithspace(IndexOffset));
      tree = attributes(tree, 'add', branch, 'IndexCount', printwithspace(IndexCount));
