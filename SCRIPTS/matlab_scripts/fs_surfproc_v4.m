function [dt_out_file tmask] = fs_surfproc_v4(dtseries_in,tmaskfile,TR,bpss_low,bpss_high,min_frames_remaining, regressors,dt_out_file_trailer,bpss_tmask,task_regressors)
...
for i = 1:length(regs(1,:))
    regs(:,i) = detrend(regs(:,i)); %Detrend all regressors
    regs(:,i) = zscore(regs(regs(:,i) ~= 0, i)); % zscore regresors
end

if(exist('task_regressors'))
    regs = [importdata(task_regressors, ' ') regs];
end
...
% MASK the regressors by the temporal mask
regs_masked = regs(tmask==1,:); % mask excluded
constant = ones(size(regs_masked,1),1);

for i = 1:length(ConstantRegressors)
    constant(regs_masked(:,ConstantRegressors(i)) ~= 0) = 0;    % set the runs with constants already to 0 so the resting state runs are done correctly
end

regs_masked = [regs_masked constant]; % add column of 1's

usable_frames = find(tmask==1);

% APPLY REGRESSORS
parfor r = 1:vox
    voxel = cii(r,usable_frames)';
    [B,~,R] = regress(voxel,regs_masked);
    cii(r,usable_frames) = R;
    betas(r,:)=B;
end
