...
HighResMesh="${11}"
LowResMesh="${12}"
AtlasTransform="$AtlasSpaceFolder/xfms/${13}"
InverseAtlasTransform="$AtlasSpaceFolder/xfms/${14}"
...
echo $Subject
echo T1wFolder="$3"
echo AtlasSpaceFolder="$4"
echo NativeFolder="$5"
echo FreeSurferFolder="$6"
echo FreeSurferInput="$7"
echo T1wImage="$8"
echo T2wImage="$9"
echo SurfaceAtlasDIR="${10}"
echo HighResMesh="${11}"
echo LowResMesh="${12}"
echo AtlasTransform="${13}"
echo InverseAtlasTransform="${14}"
echo AtlasSpaceT1wImage="${15}"
echo AtlasSpaceT2wImage="${16}"
echo T1wImageBrainMask="${17}"
echo FreeSurferLabels="${18}"
echo GrayordinatesSpaceDIR="${19}"
echo GrayordinatesResolution="${20}"
echo SubcorticalGrayLabels="${21}"
echo $CARET7DIR
...
fslmaths "$T1wFolder"/wmparc_1mm.nii.gz -bin -dilD -dilD -dilD -ero -ero "$T1wImageBrainMask"_1mm.nii.gz
${CARET7DIR}/wb_command -volume-fill-holes "$T1wImageBrainMask"_1mm.nii.gz "$T1wImageBrainMask"_1mm.nii.gz
fslmaths "$T1wImageBrainMask"_1mm.nii.gz -bin "$T1wImageBrainMask"_1mm.nii.gz

applywarp --rel --interp=nn -i "$T1wImageBrainMask"_1mm.nii.gz -r "$AtlasSpaceFolder"/"$AtlasSpaceT1wImage".nii.gz --premat=$FSLDIR/etc/flirtsch/ident.mat -o "$T1wImageBrainMask".nii.gz
applywarp --rel --interp=nn -i "$T1wImageBrainMask"_1mm.nii.gz -r "$AtlasSpaceFolder"/"$AtlasSpaceT1wImage".nii.gz -w "$AtlasTransform" -o "$T1wImageBrainMask".nii.gz
...
applywarp --interp=nn -i "$T1wFolder"/wmparc.nii.gz -r "$AtlasSpaceFolder"/ROIs/Atlas_ROIs."$GrayordinatesResolution".nii.gz -o "$AtlasSpaceFolder"/ROIs/wmparc."$GrayordinatesResolution".nii.gz
...
for Hemisphere in L R ; do
  ...
  for Surface in white pial ; do
    ...
    echo ${CARET7DIR}/wb_command -set-structure "$T1wFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii ${Structure} -surface-type $Type$Secondary
    ...
    echo ${CARET7DIR}/wb_command -surface-apply-affine "$T1wFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii "$FreeSurferFolder"/mri/c_ras.mat "$T1wFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii
    ...
    echo ${CARET7DIR}/wb_command -add-to-spec-file "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject".native.wb.spec $Structure "$AtlasSpaceFolder"/"$NativeFolder"/"$Subject"."$Hemisphere"."$Surface".native.surf.gii
...
if [ $status ] ; then
  ...
  for Map in sulc thickness curvature ; do
    ...
    echo ${CARET7DIR}/wb_command -metric-mask "$AtlasSpaceFolder"/"$Subject"."$Hemisphere"."$Map"."$HighResMesh"k_fs_LR.shape.gii "$AtlasSpaceFolder"/"$Subject"."$Hemisphere".atlasroi."$HighResMesh"k_fs_LR.shape.gii "$AtlasSpaceFolder"/"$Subject"."$Hemisphere"."$Map"."$HighResMesh"k_fs_LR.shape.gii
