...
**			Layer) facility.
** Last Update:		$Author: jon $, $Date: 2016/12/09 17:10:17 $
** Source File:		$RCSfile: dulprotocol.h,v $
...
typedef struct {
  void* proxyContext;
  int (*writeFunction)(void* proxyContext, const void* buf, int nbyte);
  int (*readFunction)(void* proxyContext, void* buf, int nbyte);
  int (*pollFunction)(void* proxyContext, int timeout);
  int (*closeFunction)(void* proxyContext);
} DUL_PROXY;
...
DUL_RequestAssociation(DUL_NETWORKKEY ** network,
		       ...
		       DUL_ASSOCIATIONKEY ** association);
CONDITION
DUL_RequestAssociationWithProxy(DUL_NETWORKKEY ** network,
		       DUL_ASSOCIATESERVICEPARAMETERS * params,
		       DUL_PROXY* proxy,
		       DUL_ASSOCIATIONKEY ** association);
CONDITION
...
DUL_PRESENTATIONCONTEXTID ctxID, unsigned char result, const char *abstractSyntax,
...
#define DUL_PERROR			FORM_COND(FAC_DUL, SEV_ERROR, 61)
