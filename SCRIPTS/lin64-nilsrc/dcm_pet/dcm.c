...
** Last Update:		$Author: jon $, $Date: 2016/12/09 16:52:00 $
** Source File:		$RCSfile: dcm.c,v $
** Revision:		$Revision: 1.1 $
** Status:		$State: Exp $
*/

static char rcsid[] = "$Revision: 1.1 $ $RCSfile: dcm.c,v $";

/* System includes have been added for compatability */
...
static CTNBOOLEAN isCPU_BIG_ENDIAN(void);


/* isCPU_BIG_ENDIAN
**
** Purpose:
**  This function returns true if the CPU is big endian and false
**  otherwise.
**
...
**
** Algorithm:
**	See endianio.c (contact Avi Snyder (avi@npg.wustl.edu)/Mohana Ramaratnam (mohanar@npg.wustl.edu))
*/

static CTNBOOLEAN isCPU_BIG_ENDIAN(void) {
	CTNBOOLEAN rtn;
	if (CPU_is_bigendian()) rtn=TRUE; else rtn=FALSE;
	return rtn;
}
		    ...
		    U32 * rtnLength, void **ctx)
{
	...
	* elementItem;
    ...
    CONDITION
	cond;
	CTNBOOLEAN cpu_is_big_endian;

	cpu_is_big_endian = isCPU_BIG_ENDIAN();
		...
		PRV_GROUP_ITEM ** groupItem)
{
	...
	*groupItem = newGroupItem;
	...
	if ((*object)->groupLengthFlag) {
	    ...
	    if (LST_Insert(&newGroupItem->elementList, groupLengthItem, LST_K_AFTER) !=
		LST_NORMAL)
		return COND_PushCondition(DCM_LISTFAILURE,
					  DCM_Message(DCM_LISTFAILURE),
					  "findCreateGroup");
		      ...
		      PRV_ELEMENT_ITEM * item)
{
	...
	if (strcmp(item->element.d.string, DICOM_TRANSFERLITTLEENDIAN) == 0) {
	    ...
	    (*object)->dataOptions = DCM_EXPLICITLITTLEENDIAN;
	    fprintf(stderr, "Warning: DICOM object has a transfer syntax "
		    "not supported by\nthis implementation: %s\n"
		    "Most likely, pixel data are compressed.    We'll "
		    "continue, but check the \nresults carefully because "
		    "they might be corrupted.\n",
		    item->element.d.string);
	}
	break;
	...
	case DCM_OW:
	    ...
	    *rtnLength += length;
	    if (element->tag == DCM_PXLPIXELDATA) {
			...
			if (byteOrder != item->byteOrder) {
				temp=(char *)b;
				for (i=0;i<(length/2);i++, temp+=2) swab2(temp);
			}
	    } else {
			(void) memcpy(b, p, length);
			if (byteOrder != BYTEORDER_SAME) {
	...
	void* ctx)
{
  ...
  if (cpu_is_big_endian)
	byteOrder = BYTEORDER_REVERSE;
  else
	byteOrder = BYTEORDER_SAME;
	 ...
	 CONDITION(*sk) (void *ctx, int offset, int flag))
{
	...
	* sequenceItem;
    ...
    printf("readFile:: %d cpu_is_big_endian\n"); 
    if (callerBuf != NULL) {
	ptr = callerBuf;
	fileFlag = FALSE;
    } else
	...
	*scannedLength = 0;
	...
	if (part10Flag) {
		...
		case DCM_EXPLICITBIGENDIAN:
		    ...
		    explicitVR = TRUE;
		    break;
		default: /*byteOrder is LITTLE_ORDER */
		if (cpu_is_big_endian)
			byteOrder = BYTEORDER_REVERSE;
		else
			byteOrder = BYTEORDER_SAME;
...
DCM_PrintSequenceList(DCM_OBJECT ** object, DCM_TAG tag)
{
	...
	printf("size: %6d offset: %6d, pixel offset: %6d\n",
	       sqObject->objectSize,
	       sqObject->offset,
...
DCM_AddFragment(DCM_OBJECT** callerObject, void* fragment, U32 fragmentLength)
{
  ...
  fragmentItem->fragment = ((unsigned char*)fragmentItem)+ sizeof(DCM_FRAGMENT_ITEM);
