...
if ($#argv != 6) then
	echo "Usage: Compute_uncorrected_mean.csh <pet input name> <start frame> <end frame> <isotope> <output name> <DoFrameAlign> <SumMethod>"
...
cat $Image:r:r".json" | awk 'BEGIN{Output = 0; }{if($1 == "\"DecayFactor\":") Output = 1;  if($1 == "\"FrameTimesStart\":") Output = 0; if(Output) print($1);}' | cut -d, -f1 >! DecayFactor
cat $Image:r:r".json" | awk 'BEGIN{Output = 0; }{if($1 == "\"FrameTimesStart\":") Output = 1;  if($1 == "\"FrameDuration\":") Output = 0; if(Output) print($1);}' | cut -d, -f1 >! FrameTimesStart
cat $Image:r:r".json" | awk 'BEGIN{Output = 0; }{if($1 == "\"FrameDuration\":") Output = 1;  if($1 == "\"FrameReferenceTime\":") Output = 0; if(Output) print($1);}' | cut -d, -f1 >! FrameDuration
cat $Image:r:r".json" | awk 'BEGIN{Output = 0; }{if($1 == "\"FrameReferenceTime\":") Output = 1;  if($1 == "\"SliceThickness\":") Output = 0; if(Output) print($1);}' | cut -d, -f1 >! FrameReferenceTime
...
if($DoFrameAlign) then
	mcflirt -in $Image -out ${Output}_mcflirt -nn_final -stats -plots -report -refvol `echo $EndFrame | awk '{print($1-1)}'`
	if($status) exit 1
else
