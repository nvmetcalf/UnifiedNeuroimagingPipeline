...
echo "Running sGTM and RBV on available PET modalities..."
	...
	mri_coreg --s Freesurfer --mov ${target_path}/Masks/FreesurferMasks/${target_patid}_orig.nii --reg ${target_path}/Masks/FreesurferMasks/${target_patid}_orig.reg.lta --dof 6
...
pushd PET/Volume
	...
	if(-e ${patid}_OE_on_orig.nii.gz) then
		...
		set ScannerName = `grep ManufacturersModelName ${SubjectHome}/dicom/$H2O[1]:r:r".json" | cut -d\" -f4`
		if($#ScannerName == 0) then
			echo "JSON for OE does not have a ManufacturersModelName tag. Cannot detect scanner point spread function."
			exit 1
		endif
		set modes_fwhm = ($modes_fwhm `grep "$ScannerName" $PP_SCRIPTS/Config/scanner_psf_fwhm.csv | head -1 | cut -d, -f2`)
	endif
	...
	if(-e ${patid}_OEF_on_orig.nii.gz) then
		...
		set ScannerName = `grep ManufacturersModelName ${SubjectHome}/dicom/$H2O[1]:r:r".json" | cut -d\" -f4`
		if($#ScannerName == 0) then
			echo "JSON for OEF does not have a ManufacturersModelName tag. Cannot detect scanner point spread function."
			exit 1
		endif
		set modes_fwhm = ($modes_fwhm `grep "$ScannerName" $PP_SCRIPTS/Config/scanner_psf_fwhm.csv | head -1 | cut -d, -f2`)
	endif
	...
	if(-e ${patid}_CMRO2_on_orig.nii.gz) then
		...
		set ScannerName = `grep ManufacturersModelName ${SubjectHome}/dicom/$H2O[1]:r:r".json" | cut -d\" -f4`
		if($#ScannerName == 0) then
			echo "JSON for CMRO2 does not have a ManufacturersModelName tag. Cannot detect scanner point spread function."
			exit 1
		endif
		set modes_fwhm = ($modes_fwhm `grep "$ScannerName" $PP_SCRIPTS/Config/scanner_psf_fwhm.csv | head -1 | cut -d, -f2`)
	endif
	...
	while($i <= $#modes_available)
		...
		lta_convert --inlta identity.nofile --src $mode --trg ${target_path}/Masks/FreesurferMasks/${target_patid}_orig.nii --outlta $mode:r:r".orig.lta" --subject Freesurfer
		if ($status) exit 1
		
		mri_concatenate_lta -subject Freesurfer $mode:r:r".orig.lta" ${target_path}/Masks/FreesurferMasks/${target_patid}_orig.reg.lta $mode:r:r".reg.lta"
		if ($status) exit 1
		
		#this is for QC and making sure the orig images get to gtm space properly
		mri_vol2vol --mov ${mode} --lta ${target_path}/Freesurfer/mri/gtmseg.lta --targ $ParcellationFilename --o ${mode:r:r}_on_gtm.nii.gz --nearest
		if ($status) exit 1
		...
		cd $modes_available[$i]_${ParcellationName}_gtmpvc
			...
			mri_vol2vol --mov rbv.nii.gz --lta ${target_path}/Freesurfer/mri/gtm_to_orig.lta --targ ${target_path}/Freesurfer/mri/orig.mgz --o rbv_to_orig.nii.gz --nearest
			if($status) exit 1
