...
pushd PET/Volume
	...
	while($i <= $#modes_available)
		...
		mri_vol2vol --mov ${mode} --lta ${SubjectHome}/Freesurfer/mri/gtmseg.lta --targ ${SubjectHome}/PET/Parcellations/gtmseg+wmparc.mgz --o ${mode:r:r}_on_gtm.nii.gz --nearest
		if ($status) exit 1
		
		mri_gtmpvc --sd ${SubjectHome} --i $mode --reg $mode:r:r".reg.lta" --psf $FWHM --seg ${SubjectHome}/PET/Parcellations/gtmseg+wmparc.mgz --default-seg-merge --auto-mask PSF .01 --mgx .01 --o $modes_available[$i]"_gtmpvc.output" --no-rescale --no-reduce-fov --rbv --rbv-res 1 --threads 6
		if ($status) exit 1
		
		python3 $PP_SCRIPTS/PET/python3/rbv_stats.py $modes_available[$i]_gtmpvc.output/aux/rbv.segmean.nii.gz $modes_available[$i]_gtmpvc.output/gtm.stats.dat $modes_available[$i]_gtmpvc.output/rbv.stats.dat
		if ($status) exit 1
		cd $modes_available[$i]_gtmpvc.output
			...
			flirt -in rbv_to_orig.nii.gz -ref ${SubjectHome}/Anatomical/Volume/T1/${patid}_T1 -out rbv_to_${patid}_T1 -init ${SubjectHome}/Masks/FreesurferMasks/${patid}_orig_to_${patid}_T1.mat -applyxfm
			if($status) exit 1
