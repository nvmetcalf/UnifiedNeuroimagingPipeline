...
foreach dcmdir (`gawk '$1=="'$MD'" {print $2}' $dcmtxt`)
	set md = `echo $MD | gawk '{print tolower($0)}'`
	set i = `gawk '$1=="'$MD'"&&$2=="'$dcmdir'" {if(NF>2)print $3; else print ""}' $dcmtxt`
	switch ($MD)
		case "FDG":
...
#	cp -r $outdir/MRAC                   $outdir/$MD$i$str/umap
#	mkdir $outdir/$MD$i$str/umap
#	mkdir $outdir/$MD$i$str/umap/frame1
#	set frame1 = `ls -1 $outdir/MRAC | sort | head -1`
#	mv $frame1 $outdir/$MD$i$str/umap/frame1
...
pushd $outdir
foreach IMG ($IMAGES)
	set img = `echo $IMG | gawk '{print tolower($0)}'`
	RunJSRecon.csh $IMG O15NAC.txt -uo $pid$img"_umap" -o $pid$img"_"$pname -m `basename $mapdrive`;	if ($status) exit $status
	t4img_4dfp none $pid$img"_"$pname $pid$img"_"$pname"_222" -O222;					if ($status) exit $status
end
popd
...
#create $pid"_ct_msk" and $pid"_ct_dfnd"
#attempt to remove non-subject features from ct by masking with mpr and clustering
...
#	t4_mul $pid"_ct_to_"$pid"_mpr_t4" $pid"_mpr_to_"$img"_t4";					if ($status) exit $status
	...
	t4img_4dfp -n $pid"_ct_to_"$img"_222_t4" $pid"_ct_dfnd" $pid"_ct_dfnd_on_"$img -O$img;		if ($status) exit $status	#likely bug: shrinks $pid"_ct_dfnd" mask at edges
...
#	set txt = `gawk '$1=="'$MD'" {t = $2} END {print t}' $reconguide`
	foreach label ($labels)
		set txt = `gawk '$1=="'$MD$label'" {t = $NF} END {print t}' $reconguide`
		...
		RunJSRecon.csh $IMG $txt.txt -ui $umap -o $img -m `basename $mapdrive`;	if ($status) exit $status
		popd
	end
end
