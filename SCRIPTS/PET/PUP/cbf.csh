...
set mpr  = `~larsc/bin/getroot.csh p7391/p7391_mpr.4dfp.img`
set mpr  = `basename $mpr`
set tof  = `~larsc/bin/getroot.csh TOF2MPR/EV001_fl3d1_TOF.4dfp.img`
#set pct  = p
set ev   = EV001
set pid  = p7391
set fwhm = 6.0
...
# Get whole brain TAC
	foreach s (1 2 3 4)
		t4img_4dfp ${pid}/${mpr}_to_${pid}${mode}${s}_sumall_t4 TOF2MPR/${mpr}_mskt Artery/${mpr}_mskt_on_${pid}${mode}${s}_sumall -O${pid}/${pid}${mode}${s}_sumall -n
...
pushd ${pid}
t4_inv ${tofbase}_to_${mpr}_t4 ${mpr}_to_${tofbase}_t4
popd
foreach s (1 2 3 4)
	
	foreach art (lArt rArt)
		t4_mul ${pid}/${pid}${mode}${s}_sumall_to_${mpr}_t4 ${pid}/${mpr}_to_${tofbase}_t4 ${pid}/${pid}${mode}${s}_sumall_to_${tofbase}_t4
