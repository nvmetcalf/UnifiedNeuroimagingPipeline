...
setenv PATH "${PATH}:/data/nil-bluearc/raichle/suy/PPG/scripts:~suy/bin"
...
#	rm temp$$.4dfp.*
	...
	foreach MD ($MDS)
		...
		if ($MD == FDG) then
			...
			cp -r MRAC_v$i $MD"_v"$i/umap
#			~suy/bin/sortncpdcm $umapdir $MD"_v"$i/umap dcm;							if ($status) exit $status
		else
			foreach n (`gawk '$1=="'$pid'"&&$2=="'$MD'"&&$4=='$i' {print $3}' ../dcm_subdir.txt`)
				...
				cp -r MRAC_v$i $MD$n"_v"$i/umap
#				~suy/bin/sortncpdcm $umapdir $MD$n"_v"$i/umap dcm;						if ($status) exit $status
				switch ($MD)
				case "HO":
