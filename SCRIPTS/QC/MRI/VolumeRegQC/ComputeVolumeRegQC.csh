...
if(-e ${SubjectHome}/Anatomical/Volume/T1/${patid}_T1_111.nii.gz && ! $?day1_path) then
	...
	if($NonLinear) then
		#MPR->NonLinAtl ETA
		matlab -nodesktop -nosplash -r "try;addpath(genpath('${PP_SCRIPTS}/matlab_scripts'));ComputeAnatomicalVolumeCorrelation('${target}.nii.gz', '${target}_brain_mask.nii.gz', '${SubjectHome}/Anatomical/Volume/T1/${patid}_T1_111_fnirt.nii.gz', '${SubjectHome}/Masks/${patid}_used_voxels_111_fnirt.nii.gz', '${SubjectHome}/QC/temp.txt');end;exit"
...
if(-e ${SubjectHome}/Anatomical/Volume/FLAIR/${patid}_FLAIR_111.nii.gz && ! $?day1_patid) then
	...
	#flair to T1
	if($NonLinear) then
		matlab -nodesktop -nosplash -r "try;addpath(genpath('${PP_SCRIPTS}/matlab_scripts'));ComputeAnatomicalVolumeCorrelation('${SubjectHome}/Anatomical/Volume/T1/${patid}_T1_111_fnirt.nii.gz', '${SubjectHome}/Masks/${patid}_used_voxels_111_fnirt.nii.gz', '${SubjectHome}/Anatomical/Volume/FLAIR/${patid}_FLAIR_111_fnirt.nii.gz', '${SubjectHome}/Masks/${patid}_used_voxels_111_fnirt.nii.gz', '${SubjectHome}/QC/temp.txt');end;exit"
		echo "Non-Linearly Aligned FLAIR -> T1 : "`cat ${SubjectHome}/QC/temp.txt` >> QC/ETA.txt
	endif
	
	rm -f ${SubjectHome}/QC/temp.txt
	matlab -nodesktop -nosplash -r "try;addpath(genpath('${PP_SCRIPTS}/matlab_scripts'));ComputeAnatomicalVolumeCorrelation('${SubjectHome}/Anatomical/Volume/T1/${patid}_T1_111.nii.gz', '${SubjectHome}/Masks/${patid}_used_voxels_111.nii.gz', '${SubjectHome}/Anatomical/Volume/FLAIR/${patid}_FLAIR_111.nii.gz', '${SubjectHome}/Masks/${patid}_used_voxels.nii.gz', '${SubjectHome}/QC/temp.txt');end;exit"
...
if($?BOLD) then
	...
	if($?day1_patid) then
		if($NonLinear) then
			matlab -nodesktop -nosplash -r "try;addpath(genpath('${PP_SCRIPTS}/matlab_scripts'));ComputeAnatomicalVolumeCorrelation('${day1_path}/Anatomical/Volume/BOLD_ref/${day1_patid}_BOLD_ref_${FinalResTrailer}_fnirt.nii.gz', '${day1_path}/Masks/${day1_patid}_used_voxels_fnirt_${FinalResTrailer}.nii.gz', '${SubjectHome}/Anatomical/Volume/BOLD_ref/${patid}_BOLD_ref_${FinalResTrailer}_fnirt.nii.gz', '${SubjectHome}/Masks/${patid}_used_voxels_${FinalResTrailer}_fnirt.nii.gz', '${SubjectHome}/QC/temp.txt');end;exit"
		else
		...
		if($NonLinear) then
			matlab -nodesktop -nosplash -r "try;addpath(genpath('${PP_SCRIPTS}/matlab_scripts'));ComputeAnatomicalVolumeCorrelation('${target}_${FinalResTrailer}_fnirt.nii.gz', '${target}_brain_mask_${FinalResTrailer}_fnirt.nii.gz', '${SubjectHome}/Anatomical/Volume/BOLD_ref/${patid}_BOLD_ref_${FinalResTrailer}_222.nii.gz', '${SubjectHome}/Masks/${patid}_used_voxels_${FinalResTrailer}_fnirt.nii.gz', '${SubjectHome}/QC/temp.txt');end;exit"
		else
