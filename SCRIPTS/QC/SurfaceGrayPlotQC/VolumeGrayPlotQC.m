function VolumeGrayPlotQC(sub, tmaskfile, run_boundaries_filename, resid_bpss_nifti, aligned_nifti, volume_ribbon_mask, TR, fdfilename, dvarfilename, FigureFilenameRoot)
...
%     end
    ...
    P_voxels = [];
    Voxels = randi(length(resid_nifti(:,1)), 500,1);
    for j = 1:length(run_start_frames)
        ...
        run = resid_nifti(Voxels,run_start:run_end);
    ...
    %xlim(F);   %set max frequency to 1/2 sampling frequency (TR)
    ...
    for j = 1:length(run_start_frames)
        ...
        run = raw_nifti(Voxels,run_start:run_end);
        run_tmask = tmask(run_start:run_end);
